...
import encry.explorer.env.ContextClientQueues
import fs2.Stream
...
object GatheringService {
  ...
  def apply[F[_]: Sync: Parallel: Logger](clientService: ClientService[F])(
    implicit clientQC: ContextClientQueues[F]
  ): F[GatheringService[F]] =
    clientQC
      .ask(_.unreachableUrlsQueue)
      .map(unreachableUrlsQueue =>
                ...
                case Some(url) =>
                  f(url).flatMap {
                    case Right(potentialElement) =>
                      Logger[F]
                        .debug(s"Got expected element from $url")
